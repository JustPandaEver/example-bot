            ...
            case prefix + 'ytmp3': {
                ...
                await api.ytmp3(args[1])
                    .then(res => {
                        ...
                        conn.sendMessage(from, { url: res.audio }, 'documentMessage', { quoted: msg, contextInfo: { externalAdReply: { title: res.data.result.title, mediaType: 2, thumbnailUrl: res.data.result.thumb, mediaUrl: res.data.result.metaData.url }}})
                    })
                    .catch(err => {
                        console.log(err)
                        conn.reply(from, require('util').format(err), msg)
